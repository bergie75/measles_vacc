...
# Convert to numpy array directly to guarantee (120, 120)
adjacent_matrix = adjacent_matrix.values
...
if __name__ == "__main__":
    ...
    if run_through:
        ...
        if isinstance(adjacent_matrix, pd.DataFrame):
            adj_data = adjacent_matrix.iloc[:num_patches, :num_patches].values
        else:
            adj_data = adjacent_matrix[:num_patches, :num_patches]
            
        multi_beta = generating_beta_matrix(adj_data, beta, in_cluster_strength=0.5)
        ...
        if cluster_first:
            clustering_scenario(tag, disease_params, patch_pop, clusters, sensitivities=sensitivities, 
                                num_days=num_days, detection_day_lag=detect_lag)
...
#another modifcation from the original code to simply reduce test run times
            ...
            for i in range(0, num_samples):
                ...
                cumulative_case_counts_sample, clusters, det_day, det_patch = vaccination_strategy_better_wes_model(
                    tag, deepcopy(abridged_disease_params), patch_pop, 
                    ...
                    days_to_disperse_stockpile=stockpile_dispersal_day)
