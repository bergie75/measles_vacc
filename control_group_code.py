...
def vaccination_strategy_control_group(abridged_disease_params, patch_populations, extra_doses_per_day, multi_beta, num_days=600, chosen_patch=0):
    ...
    pop_state[2*num_patches+chosen_patch] += frac_exposed

    # Main daily epidemic simulation loop
    for day in range(0, num_days):
        # Reset to baseline every day
        multi_alpha = np.copy(multi_alpha)
    ...
    track_cumulative_totals = pop_state[4 * num_patches : 5 * num_patches]    
...
if __name__ == "__main__":
    ...
    save_folder = script_dir / "test_control_group"
    save_folder.mkdir(parents=True, exist_ok=True)
    np.save(os.path.join(str(save_folder), "beta_0.001_total_cases.npy"), total_control_case_counts)
